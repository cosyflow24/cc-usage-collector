...
async function upload(result) {
  ...
  const sessionRows = result.sessions.filter((s) => isWorkAccount(s.user)).map((s) => ({
    ...
    active_ms: Math.round(s.activeTimeHours * 36e5)
  }));
  const dailyRows = result.daily.filter((d) => isWorkAccount(d.user)).map((d) => ({
    ...
    active_ms: Math.round(d.activeTimeHours * 36e5)
  }));
  if (sessionRows.length) {
    const { error } = await sb.from("cc_sessions").upsert(sessionRows, { onConflict: "user_id,session_id" });
    if (error) throw new Error(`cc_sessions upsert failed: ${error.message}`);
  }
  if (dailyRows.length) {
    const { error } = await sb.from("cc_daily").upsert(dailyRows, { onConflict: "user_id,day" });
