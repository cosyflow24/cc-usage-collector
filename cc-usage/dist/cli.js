...
async function fetchCcusageCost(since, until) {
  try {
    ...
    const list = Array.isArray(parsed.session) ? parsed.session : [];
    const map = /* @__PURE__ */ new Map();
    for (const s of list) {
...
async function fetchCcusageDailyTotal(since, until) {
  try {
    const { stdout } = await execFileAsync(
      "npx",
      [
    ...
    const parsed = JSON.parse(stdout);
    const list = Array.isArray(parsed.daily) ? parsed.daily : [];
...
function parseLine(line) {
  ...
  return {
    ...
    outputTokens: num2(usage.output_tokens),
...
program2.name("cc-usage").description("Analyze Claude Code + Codex session logs; usage and task attribution.").option("-s, --since <iso>", "start of range (ISO date/datetime)").option("-u, --until <iso>", "end of range (ISO date/datetime)").option("-d, --days <n>", "look back N local days (default: 1 = yesterday)").option("--user <id>", "override user identity (default: git email)").option("--idle-gap <min>", "idle gap minutes for active time", String(DEFAULT_IDLE_GAP_MIN)).option("--project <name>", "only include sessions from this project (cwd basename)").option("--no-commit-scan", "do not scan git commits for Jira keys").option("--json", "output JSON instead of a table").option("--upload", "upsert results (prefers ingest URL+token, else Supabase)").option(
  ...
  const idleGapMs = Number.parseInt(opts.idleGap, 10) * 6e4;
  ...
  if (opts.ccusageCheck) {
    ...
    if (ccTotal === null) {
      ...
      process.stderr.write(
        `ccusage-check: ours $${result.notionalCostUsd.toFixed(2)} vs ccusage $${ccTotal.toFixed(2)} (delta $${delta.toFixed(2)}, ${pct.toFixed(1)}%)${Math.abs(pct) > 5 ? " \u2014 DRIFT >5%, sync-check pricing.ts rates" : ""}
`
      );
    }
  }
  if (opts.upload) {
    const { applyUntaggedPolicy, httpUpload } = await import("./upload-3CSC3NNG.js");
    const toUpload = applyUntaggedPolicy(result);
    const unassigned = result.sessions.filter((s) => !s.jiraKey).length;
    if (unassigned > 0) {
      process.stderr.write(
        toUpload === result ? `${unassigned} session(s) uploaded as Unassigned (no jira key).
