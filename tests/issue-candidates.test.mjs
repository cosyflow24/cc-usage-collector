...
test("SessionStart offers cached open issues by their real titles", () => {
  ...
  try {
    ...
    const on = runSessionStart(sb);
    assert.match(on, /KI-950 — PDF nach Excel Kaskade \(In Arbeit\)/);
...
test("Codex gets the same candidates and still names its own ask tool", () => {
  ...
  try {
    seedCache(sb, ISSUES);
    const out = runSessionStart(sb, { env: { CODEX_THREAD_ID: "sid-cand" } });
